...
bool HardwareDisplayController::ModesetCrtc(const DrmOverlayPlane& primary,
  ...
  for (const auto& controller : crtc_controllers_) {
    ...
    status &=
        controller->Modeset(primary, modeset_mode, &owned_hardware_planes_);
  }
...
bool HardwareDisplayController::ScheduleOrTestPageFlip(
  ...
  for (const auto& controller : crtc_controllers_) {
    status &=
        controller->AssignOverlayPlanes(&owned_hardware_planes_, pending_planes,
                                        /*is_modesetting=*/false);
  }

  status &= GetDrmDevice()->plane_manager()->Commit(
      &owned_hardware_planes_, /*modeset=*/false, page_flip_request, out_fence);
