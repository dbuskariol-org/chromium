...
SupportedResolutionRangeMap GetSupportedD3D11VideoDecoderResolutions(
  ...
  auto& h264_profile = supported_resolutions[H264PROFILE_BASELINE];
  ...
  supported_resolutions[H264PROFILE_MAIN] =
      supported_resolutions[H264PROFILE_HIGH] = h264_profile;
  ...
  for (UINT i = 0; i < profile_count; i++) {
    ...
    if (profile_id == D3D11_DECODER_PROFILE_H264_VLD_NOFGT) {
      ...
      if (!result.max_landscape_resolution.IsEmpty()) {
        supported_resolutions[H264PROFILE_MAIN] =
            supported_resolutions[H264PROFILE_HIGH] =
                supported_resolutions[H264PROFILE_BASELINE] = result;
      }
      continue;
    }
