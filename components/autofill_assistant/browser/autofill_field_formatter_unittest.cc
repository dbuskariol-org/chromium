...
TEST(AutofillFieldFormatterTest, CreditCard) {
  ...
  EXPECT_EQ(*FormatString("51", CreateAutofillMappings(credit_card, "en-US")),
            "John Doe");

  // CREDIT_CARD_NUMBER
  EXPECT_EQ(*FormatString("52", CreateAutofillMappings(credit_card, "en-US")),
            "4111111111111111");
...
TEST(AutofillFieldFormatterTest, SpecialCases) {
  ...
  EXPECT_EQ(FormatString("-1", CreateAutofillMappings(profile, "en-US")),
            base::nullopt);
  EXPECT_EQ(FormatString(base::NumberToString(autofill::MAX_VALID_FIELD_TYPE),
                         CreateAutofillMappings(profile, "en-US")),
            base::nullopt);
...
TEST(AutofillFieldFormatterTest, DifferentLocales) {
  ...
  for (const auto& value : mappings) {
    LOG(ERROR) << value.first << ": " << value.second;
  }
...
TEST(AutofillFieldFormatterTest, AddsAllProfileFields) {
  std::map<int, std::string> expected_values = {
      {3, "Alpha"},
      {4, "Beta"},
      {5, "Gamma"},
      {6, "B"},
...
TEST(AutofillFieldFormatterTest, AddsAllCreditCardFields) {
  std::map<int, std::string> expected_values = {{-5, "Visa"},
                                                {-4, "1111"},
                                                {-2, "visa"},
                                                {51, "Alpha Beta Gamma"},
                                                {52, "4111111111111111"},
