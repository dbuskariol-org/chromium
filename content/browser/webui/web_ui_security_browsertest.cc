...
// XHR considers web to chrome-untrusted:// as a cross-origin request, and
// chrome-untrusted:// does not support cross-origin request.
IN_PROC_BROWSER_TEST_F(WebUISecurityTest,
  ...
  const std::string web_host = url::Origin::Create(web_url).Serialize();
  EXPECT_EQ(console_observer.GetMessageAt(0),
            base::StringPrintf(
                "Access to XMLHttpRequest at '%s' from origin '%s' has been "
                "blocked by CORS policy: Cross origin requests are only "
                "supported for protocol schemes: http, data, chrome, https.",
                untrusted_url.spec().c_str(), web_host.c_str()));
}
