...
BackgroundImageGeometry::BackgroundImageGeometry(
    ...
    bool root_element_has_transform)
    : box_(view),
      positioning_box_(view.RootBox()),
      painting_view_(true),
      root_element_has_transform_(root_element_has_transform) {
...
void BackgroundImageGeometry::ComputePositioningArea(
  ...
  if (ShouldUseFixedAttachment(fill_layer)) {
    ...
    // to encode paint offset in the local transform space. This is not the case
    // for the view which expands to fill the viewport but paints from the root
    // element's transform space and needs to use an offset as if painting
    // started from the view.
    if (painting_view_) {
      auto& document = container->GetDocument();
      const auto* root = document.documentElement()->GetLayoutObject();
      auto offset = root->FirstFragment().PaintOffset().ToLayoutPoint();
      if (root_element_has_transform_) {
        DCHECK(offset == LayoutPoint::Zero());
        offset -= paint_rect.Location();
      }
      unsnapped_box_offset += offset;
      snapped_box_offset += offset;
    }
  }
}
