...
public class Tab
    ...
    public long getCurrentRootTaskID() {
        if (mWebContents == null || mWebContents.getNavigationController() == null) return -1;
        NavigationController controller = mWebContents.getNavigationController();
        int rootIndex = getRootIndexForIndex(controller.getLastCommittedEntryIndex());
        return controller.getEntryAtIndex(rootIndex).getTaskID();
    }

    public long getPreviousRootTaskID() {
        if (mWebContents == null || mWebContents.getNavigationController() == null) return -1;
        NavigationController controller = mWebContents.getNavigationController();
        int rootIndex = getRootIndexForIndex(controller.getLastCommittedEntryIndex());
        if (rootIndex == 0) return -1;
        int previousRootIndex = getRootIndexForIndex(rootIndex - 1);
        return controller.getEntryAtIndex(previousRootIndex).getTaskID();
    }

    private int getRootIndexForIndex(int index) {
        if (mWebContents == null || mWebContents.getNavigationController() == null) return -1;
        NavigationController controller = mWebContents.getNavigationController();
        int activeIndex = index;
        while (activeIndex != 0 || controller.getEntryAtIndex(activeIndex).getParentTaskID() != -1) {
            activeIndex--;
        }
        return activeIndex;
    }
