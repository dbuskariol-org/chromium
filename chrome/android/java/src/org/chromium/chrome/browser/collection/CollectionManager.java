...
public class CollectionManager {
    ...
    public CollectionManager(
        ...
        final TabModelObserver tabModelObserver = new EmptyTabModelObserver() {
            ...
            public void didSelectTab(Tab tab, @TabModel.TabSelectionType int type, int lastId) {
                ...
                if (hasItems) {
                    // TODO: hacky delay to wait until onSceneChange() is done in ActivityTabProvider,
                    // and all animations to settle down.
                    // Might be moot after converting to infobar.
                    ThreadUtils.postOnUiThreadDelayed(() -> showBottomSheet(mFragmentManager, tab),
                            600);
                }
            }
        };
    ...
    public void toggleItem(String json) {
        ...
        if (!COLLECT_TO_LIST) {
            ...
            tab.getTabModelSelector().openNewTab(
    ...
    private void extractItems(final Tab tab, Boolean showOverlay,
            Callback<Boolean> hasItemsCallback) {
        if (!isAllowedUrl(tab.getUrl())) return;
        if (tab.getWebContents() == null) return;
