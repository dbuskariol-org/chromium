...
public class TabGroupLayoutTabInfo implements LayoutTabInfo {
    ...
    private int mLabelYOffsetInPixels;
    ...
    public void updateLayer(TabContentManager tabContentManager, ResourceManager resourceManager) {
        ...
        if (tabGroupTabs.length > 2) {
            nativePutTabGroupPreviewActionLayer(
    ...
    public boolean checkForClick(int x, int y, ClickCallback tabSwitchCallback,
            ...
            ClickCallback viewTabGroupCallback) {
        if (hitTestCreateTabGroup(x, y)) {
            if (mLayoutTabGroupTabs.length == 2) {
                RecordUserAction.record("TabGroup.Created.TabSwitcher");
                newTabInGroupCallback.run(mFocusedTabId);
                return true;
            } else {
                Log.i("MeilUma", "TabGroup.ExpandedFromTabSwitcher");
                RecordUserAction.record("TabGroup.ExpandedFromTabSwitcher");
                viewTabGroupCallback.run(mFocusedTabId);
                return true;
            }
        }
    ...
    private native void nativePutTabGroupPreviewActionLayer(long nativeTabGroupLayoutTabInfo,
            TabContentManager tabContentManager, float x, float y);
