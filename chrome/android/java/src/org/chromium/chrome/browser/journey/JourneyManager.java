...
public class JourneyManager {
    ...
    private void emptyJourneyFetched(long sourceTimestamp) {
        ...
        if (count == null || count < 6) {
            ...
            android.util.Log.e("Yusuf","Posting delayed fetch for "+sourceTimestamp+" after empty journey received " + count+" times due to no timestamp");
            count++;
    ...
    public void reportEmptyJourney(Tab tab) {
        String recordedValue;
        long timestamp = tab.getCurrentRootTimestamp();
        if (timestamp == 0) {
            recordedValue = "NoRootTimestamp";
            RecordUserAction.record("Autotabs.MissingJourney.NoRootTimestamp");
            android.util.Log.e("journey_info","Reporting no tabs shown due to "+recordedValue);
            return;
        }
        Journey journey = mTimestampToJourneyMap.get(timestamp);
        if (journey == null) {
            recordedValue = "NoJourneyWithTimestamp";
            RecordUserAction.record("Autotabs.MissingJourney.NoJourneyWithTimestamp");
            android.util.Log.e("journey_info","Reporting no tabs shown due to "+recordedValue);
            return;
        }
        if (journey.getTimestampsList(tab).isEmpty()) {
            if (journey.isEmpty()) {
                if (mRetryCountMap.containsKey(timestamp) && mRetryCountMap.get(timestamp) > 0) {
                    recordedValue = "RetriedDueToNoTimestamp";
                    RecordUserAction.record("Autotabs.MissingJourney.RetriedDueToNoTimestamp");
                } else {
                    recordedValue = "EmptyWithNoRetry";
                    RecordUserAction.record("Autotabs.MissingJourney.EmptyWithNoRetry");
                }
            } else if (!journey.hasJourneyInfo()) {
                recordedValue = "SinglePageJourney";
                RecordUserAction.record("Autotabs.MissingJourney.SinglePageJourney");
            } else {
                recordedValue = "MultiplePageJourneyWithEmptyList";
                RecordUserAction.record("Autotabs.MissingJourney.MultiplePageJourneyWithEmptyList");
            }
            android.util.Log.e("journey_info","Reporting no tabs shown due to "+recordedValue);
        }
    }
