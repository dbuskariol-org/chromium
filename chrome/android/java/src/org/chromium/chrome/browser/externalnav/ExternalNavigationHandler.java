...
public class ExternalNavigationHandler {
    ...
    private static final String ACTION_COLLECT = "com.google.chrome.COLLECT";
     ...
     */
    private boolean handleStar(Intent intent) {
        // FIXME: consider using addTrustedIntentExtras to make sure the intent is generated by us.
        final String action = intent.getAction();
        if (DEBUG) Log.i(TAG, "Action = %s", action);
        if (!ACTION_COLLECT.equals(action)) return false;

        final String data = intent.getDataString();
        if (DEBUG) Log.i(TAG, "Data = %s", data);
        String decoded = Uri.decode(data);
        if (DEBUG) Log.i(TAG, "URL decoded data = %s", decoded);
        CollectionManager collectionManager = CollectionManager.getInstance();
        if (collectionManager != null) {
            // FIXME: do not directly decode JSON from the web!
            collectionManager.toggleItem(decoded);
        }
        return true;
    }
    ...
    private @OverrideUrlLoadingResult int shouldOverrideUrlLoadingInternal(
        ...
        if (handleStar(intent)) {
            if (DEBUG) Log.i(TAG, "OVERRIDE_WITH_ASYNC_ACTION: Star handled");
            return OverrideUrlLoadingResult.OVERRIDE_WITH_ASYNC_ACTION;
        }
