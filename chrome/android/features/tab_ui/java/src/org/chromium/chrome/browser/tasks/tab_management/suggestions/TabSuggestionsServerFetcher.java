    ...
    public void fetch(TabContext tabContext, Callback<List<TabSuggestion>> callback) {
        ...
        try {
            for (Tab tab : tabContext.getTabs()) {
                ...
                if (tab.getReferrer() != null) {
                    jsonTab.put(REFERRER_KEY, tab.getReferrer().getUrl());
                }
        ...
        try {
            ...
            for (int i = 0; i < jsonResponse.length(); i++) {
                ...
                for (int j = 0; j < jsonTabs.length(); j++) {
                    ...
                    JSONArray jsonTabGroup = jsonTabs.getJSONArray(j);
                    for (int k = 0; k < jsonTabGroup.length(); k++) {
                        JSONObject jsonTab = jsonTabGroup.getJSONObject(k);
                        // TODO(crbug/958512) Replace when TabContext removes its reliance on Tab
                        tabs.add(new Tab(-1, null, false, null, null, null,
                                new LoadUrlParams(jsonTab.getString(URL_KEY), 1)));
                    }
