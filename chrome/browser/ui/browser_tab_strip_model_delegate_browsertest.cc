...
IN_PROC_BROWSER_TEST_F(BrowserTabStripModelDelegateTest, MoveTabToNewWindow) {
  ...
  // single tab window.
  EXPECT_FALSE(delegate->CanMoveTabToWindow(0));

  AddTabAtIndex(1, url2, ui::PAGE_TRANSITION_LINK);

  EXPECT_TRUE(delegate->CanMoveTabToWindow(0));
  EXPECT_TRUE(delegate->CanMoveTabToWindow(1));
