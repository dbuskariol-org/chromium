...
TYPED_TEST_P(ContainerOpsTest, TestExtractRandomSubset_Merge) {
  ...
  // Moved element disappears. Assuming the underlying set implementation is
  // correct (we'd be in pretty bad shape if that were not true) verifies that
  // the element being moved is one we expect.
  EXPECT_TRUE(ExtractRandomSubset(&from, &to, 1));
  EXPECT_EQ(3u, from.size());
  EXPECT_EQ(5u, to.size());
}
