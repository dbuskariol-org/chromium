...
// Test must be disabled on linux-chromeos-chrome because it fails there. See
// crbug.com/1061548.
#if defined(OS_CHROMEOS)
#define MAYBE_PageLCPStopsUponInput DISABLED_PageLCPStopsUponInput
#else
#define MAYBE_PageLCPStopsUponInput PageLCPStopsUponInput
#endif
IN_PROC_BROWSER_TEST_F(PageLoadMetricsBrowserTest,
                       MAYBE_PageLCPStopsUponInput) {
  embedded_test_server()->ServeFilesFromSourceDirectory("content/test/data");
  content::SetupCrossSiteRedirector(embedded_test_server());
  ASSERT_TRUE(embedded_test_server()->Start());
