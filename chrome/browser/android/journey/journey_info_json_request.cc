...
JourneyInfoJsonRequest::Builder::Builder()
    : url_(GURL("https://chrome-memex-dev.appspot.com/api/"
                "journey?id=527860506446225765")){};
...
JourneyInfoJsonRequest::Builder& JourneyInfoJsonRequest::Builder::SetTimestamps(
  ...
  std::string timestamps_string;
  for (int64_t timestamp : timestamps) {
    timestamps_string += std::to_string(timestamp) + ",";
  }
  timestamps_string.pop_back();

  url_ = GURL(
      "https://chrome-memex-dev.appspot.com/api/"
      "journey_from_pageload?page_timestamp_usec=" + timestamps_string + "&server_label=study");
...
JourneyInfoJsonRequest::Builder& JourneyInfoJsonRequest::Builder::SetContentUrl(
    const GURL& url) {
  content_url_ = url;
  return *this;
}
...
JourneyInfoJsonRequest::Builder::BuildURLFetcher(
  ...
  std::unique_ptr<net::URLFetcher> url_fetcher = net::URLFetcher::Create(
      url_, net::URLFetcher::GET, delegate, traffic_annotation);
