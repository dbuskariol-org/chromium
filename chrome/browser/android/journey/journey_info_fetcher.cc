...
namespace {
const char kChromeSyncScope[] = "https://www.googleapis.com/auth/chromesync";
}  // namespace
...
void JourneyInfoFetcher::AccessTokenFetchFinished(
  ...
  while (!pending_requests_.empty()) {
    std::pair<std::vector<int64_t>, JourneyInfoAvailableCallback> timestamp_and_callback =
        std::move(pending_requests_.front());
    pending_requests_.pop();
    StartRequest(timestamp_and_callback.first,
                 std::move(timestamp_and_callback.second), access_token);
  }
}
...
void JourneyInfoFetcher::JsonRequestDone(
  ...
  for (const auto& journey : *journeys) {
    ...
    const base::ListValue* source_page_timestamps;
    dict->FindKey("source_page_timestamp_usec")->GetAsList(&source_page_timestamps);

    std::string journey_id = "";
    std::vector<PageLoadInfo> pageloads;

    if (dict->FindKey("status")->GetString().compare("STATUS_PAGELOAD_NOT_FOUND") == -1) {
      const base::ListValue* autotabs_list;
      if (selection_type_ != -1
          && dict->FindKey("experimental_autotabs")->GetList().size() > (unsigned int) selection_type_
          && dict->FindKey("experimental_autotabs")->GetList()[selection_type_].FindKey("pageloads")) {
        dict->FindKey("experimental_autotabs")->GetList()[selection_type_].FindKey("pageloads")->GetAsList(&autotabs_list);
        VLOG(0) << "Selecting experimental autotabs " << selection_type_;
      } else {
        dict->FindKey("default_autotabs")->FindKey("pageloads")->GetAsList(&autotabs_list);
      }
    ...
    for (const auto& source : *source_page_timestamps) {
      int64_t source_timestamp = std::stoll(source.GetString());
      VLOG(0) << "Yusuf Source timestamp is found and is " << source_timestamp<< " also pageloads size is "<<pageloads.size();
      std::move(callback).Run(source_timestamp, pageloads, journey_id);
    }
  }
}
