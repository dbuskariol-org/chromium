...
void TabGroupContentLayer::SetProperties(
    ...
    int inset_diff,
    ui::NinePatchResource* selected_tab_border_inner_shadow_resource) {
  ...
  for (int i = 0; i < size; i++) {
    ui::NinePatchResource* tab_border_inner_shadow_resource =
        border_inner_shadow_resource;
    if (tab_ids[i] == id) {
      tab_border_inner_shadow_resource =
          selected_tab_border_inner_shadow_resource;
    }
    group_tab_content_layers_[i]->SetProperties(
        ...
        tab_border_inner_shadow_resource, 1, width, height, tab_ids,
