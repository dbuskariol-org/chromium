...
void TabGroupLayoutTabInfo::PutTabGroupPreviewActionLayer(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jobj,
    const base::android::JavaParamRef<jobject>& j_tab_contents_manager,
    jfloat width,
  ...
  scoped_refptr<cc::UIResourceLayer> preview_action_layer =
      tab_content_manager->CreateTabGroupPreviewActionLayer(width);

  preview_action_layer->SetPosition(gfx::PointF(0, y));
  own_tree_->AddChild(preview_action_layer);
}
