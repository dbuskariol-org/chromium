...
scoped_refptr<cc::UIResourceLayer> TabGroupLayer::CreateTabGroupActionLayer(
    float dp_to_px,
    float width,
    std::string button_string,
    std::string description,
    bool show_description) {
  scoped_refptr<cc::UIResourceLayer> tab_group_action_layer =
      cc::UIResourceLayer::Create();

  tab_group_action_layer->SetIsDrawable(true);

  float label_text_height = dp_to_px * 12;

  // TODO(meiliang): Create the pill shape button with given button text
  // button layer
  ...
  tab_group_action_layer->AddChild(button_layer);

  // Description layer
  if (show_description) {
    scoped_refptr<cc::UIResourceLayer> description_layer =
        cc::UIResourceLayer::Create();

    description_layer->SetIsDrawable(true);
    // Width is in pixels!
    description_layer->SetBounds(gfx::Size(width, 12 * dp_to_px));

    SkBitmap label_bitmap;
    label_bitmap.allocN32Pixels(description_layer->bounds().width(),
                                description_layer->bounds().height());
    label_bitmap.eraseColor(SK_ColorTRANSPARENT);

    cc::SkiaPaintCanvas label_paint_canvas(label_bitmap);
    gfx::Canvas label_canvas(&label_paint_canvas, 1.0f);
    label_canvas.Save();
    label_canvas.ClipRect(
        gfx::Rect(label_bitmap.width(), label_bitmap.height()));

    auto label_render_text = CreateRenderText(
        label_text_height, gfx::Font::Weight::NORMAL,
        SkColorSetARGB(138, 0, 0, 0), false,
        gfx::Rect(label_bitmap.width(), label_bitmap.height()));

    label_render_text->SetHorizontalAlignment(gfx::ALIGN_CENTER);
    base::string16 description_text = base::string16();
    base::UTF8ToUTF16(description.c_str(), description.length(),
                      &description_text);
    label_render_text->SetText(description_text);
    label_render_text->Draw(&label_canvas);
    label_canvas.Restore();
    label_bitmap.setImmutable();
    description_layer->SetBitmap(label_bitmap);
    tab_group_action_layer->AddChild(description_layer);

    description_layer->SetPosition(gfx::PointF(
        button_layer->position().x(),
        button_layer->position().y() + button_layer->bounds().height() * 2));
  }

  return tab_group_action_layer;
}
...
scoped_refptr<cc::UIResourceLayer> TabGroupLayer::CreateTabGroupCreationLayer(
  ...
  if (!kCache) {
    kCache =
        CreateTabGroupActionLayer(dp_to_px, width, "CREATE TAB GROUP",
                                  "Quickly switch between related tabs", false);
  }
  return kCache;
}
...
TabGroupLayer::CreateTabGroupPreviewActionLayer(float dp_to_px, float width) {
  static scoped_refptr<cc::UIResourceLayer> kCache;
  if (!kCache) {
    kCache = CreateTabGroupActionLayer(dp_to_px, width, "VIEW TAB GROUP",
                                       "View tabs within this group", false);
  }
  return kCache;
}
